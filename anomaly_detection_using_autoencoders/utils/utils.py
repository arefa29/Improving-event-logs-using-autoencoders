...
def calculateCumulativeDuration(df):
...
def OneHotEncoding(df, categorical_variables):
    for i in categorical_variables:
        encoded_df = pd.get_dummies(df, columns=categorical_variables,
                                drop_first = False)
    return encoded_df

def getLongestLength(groupByCase):
    '''Returns the length of longest case'''
    max_len = 1
    for case, group in groupByCase:
        curr_len = group.shape[0]
        if curr_len > max_len:
            max_len = curr_len
    return max_len

def padWithZeros(vector, maxlen):
    '''Returns the vector padded with zeros with maxlen rows'''
...
def getProbability(recon_test):
    ...
    for i in range(recon_test.size(0)):
        cont_values = recon_test[i, :, 0].contiguous().view(recon_test.size(1),1) #(14,1) #(35,1)
        softmax_values = softmax(recon_test[i, :, 1:])
        if i == 0:
            recon = torch.cat([cont_values, softmax_values], 1)
            recon = recon.contiguous().view(1,recon_test.size(1), recon_test.size(2)) #(1, 14, 15) ->for small log, #(1, 35, 8)
        else:
